  ...
  return (
    <div className="w-full h-full flex flex-col items-center pt-8 pb-4">
      <div className="bg-white/95 dark:bg-[#111522]/95 border border-[#e2d8cc] dark:border-white/10 rounded-3xl max-w-[1240px] p-8 w-full flex-grow flex flex-col overflow-hidden animate-[fadeIn_0.3s_ease-out] shadow-[0_20px_50px_rgba(180,140,100,0.12)] dark:shadow-[0_20px_50px_rgba(0,0,0,0.4)]">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-amber-800 dark:text-blue-400 text-xs font-mono font-bold uppercase tracking-wider">Step 5 of 7</span>
            </div>
            <h2 className="text-2xl font-extrabold text-stone-900 dark:text-white tracking-tight">
              Software Suite Bundles
            </h2>
            <p className="text-stone-600 dark:text-slate-400 text-xs mt-0.5">
          ...
          <div className="flex items-center gap-3">
            <button 
              onClick={onBack}
              className="px-4 py-2 rounded-xl bg-[#f0ebe1] hover:bg-[#e4ddce] text-stone-800 dark:bg-white/5 dark:hover:bg-white/10 dark:text-white font-semibold text-xs border border-[#ded3c4] dark:border-white/10 transition-colors"
            >
              Back
            </button>
            <button 
              onClick={onNext}
              className="px-5 py-2 rounded-xl bg-amber-800 hover:bg-amber-900 dark:bg-blue-600 dark:hover:bg-blue-500 text-white font-semibold text-xs shadow-[0_2px_10px_rgba(180,100,50,0.35)] dark:shadow-[0_2px_10px_rgba(59,130,246,0.4)] transition-all"
            >
          ...
          {BUNDLES.map((group: any) => (
            <div key={group.name} className="mb-6 last:mb-0">
              <div className="flex items-center gap-2 mb-3">
                <Package className="w-4 h-4 text-amber-700 dark:text-blue-400" />
                <h3 className="text-sm font-bold text-stone-900 dark:text-white uppercase tracking-wider font-mono">
                  {group.name}
                </h3>
              </div>
                  ...
                  return (
                    <button
                      ...
                      className={`text-left p-3.5 rounded-2xl border transition-all duration-150 relative group ${
                        isSelected 
                          ? 'bg-amber-500/10 border-amber-700 dark:bg-blue-500/10 dark:border-blue-500 shadow-[0_0_15px_rgba(180,100,50,0.15)] dark:shadow-[0_0_15px_rgba(59,130,246,0.15)]' 
                          : 'bg-[#fbf8f3] dark:bg-white/[0.02] border-[#ebe3d5] dark:border-white/10 hover:border-amber-700/30 dark:hover:border-white/20 hover:bg-[#f6f0e4] dark:hover:bg-white/[0.04]'
                      }`}
                    >
                      <div className="flex justify-between items-start mb-1">
                        ...
                        <div className={`w-4 h-4 rounded-md border flex items-center justify-center transition-colors shrink-0 ${
                          isSelected ? 'bg-amber-800 border-amber-800 dark:bg-blue-500 dark:border-blue-500 text-white' : 'border-[#dcd2c4] dark:border-white/20 bg-white dark:bg-black/40'
                        }`}>
                          {isSelected && <Check className="w-3 h-3 stroke-[3]" />}
                        </div>
                      </div>
                      <span className="text-[10px] text-stone-500 dark:text-slate-400 font-mono block truncate">{pkg.id}</span>
                    </button>
                  );
                })}
              </div>
        ...
        {/* Footer Navigation */}
        <div className="flex justify-between items-center pt-4 border-t border-[#ebe3d5] dark:border-white/10 mt-auto">
          <button 
